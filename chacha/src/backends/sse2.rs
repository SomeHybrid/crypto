...
pub fn hchacha(key: &[u8], nonce: &[u8]) -> [u8; 32] {
    unsafe {
        let state = [
            _mm_loadu_si128(SIGMA.as_ptr() as *const __m128i),
            _mm_loadu_si128(key.as_ptr() as *const __m128i),
        ...
        for _ in 0..10 {
            out = double_quarter_round(out);
        }

        let mut output = [0u8; 32];

        _mm_storeu_si128(output.as_mut_ptr() as *mut __m128i, out[0]);

        _mm_storeu_si128(output[16..].as_mut_ptr() as *mut __m128i, out[3]);
